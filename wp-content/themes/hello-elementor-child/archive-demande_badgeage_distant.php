...
//var_dump($users_ids);
if (in_array("um_custom_role_1", $user->roles) /*&& $_GET['demandes']=='les_miennes'*/)	{
	$arg = array(
		"author__in"=>$user_id,
...
} elseif	(in_array("um_chef-de-service", $user->roles) && $_GET['demandes']=='mon_service'){
	$arg = array(
		"author__in"=>$users_service_ids,
		"post_type"=>"demande_badgeage",
		//"meta_query"=>["key"=>"service"]
	);
} elseif	(in_array("um_directeur", $user->roles) && $_GET['demandes']=='direction'){
	$arg = array(
		"author__in"=>$users_service_ids,
		"post_type"=>"demande_badgeage",
		"meta_query"=>[["key"=>"decision_responsable","value"=>"valider"]]
	);
} elseif	(in_array("um_directeur", $user->roles) && $_GET['demandes']=='toutes'){
	$arg = array(
		"author__in"=>$allusers_ids,
		"post_type"=>"demande_badgeage"
	);
} 
 elseif	(in_array("um_ressources-humaine", $user->roles) && $_GET['demandes']=='rh'){
	$arg = array(
		//"author__in"=>$users_service_ids,
		"post_type"=>"demande_badgeage",
